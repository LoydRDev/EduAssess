...
public class AdminInstructorAssignmentController implements Initializable {
    ...
    private void loadData() {
        ...
        instructorComboBox.setItems(FXCollections.observableArrayList(
                instructors.stream()
                        .map(instructor -> {
                            String idNumber = instructor.get("id_number");
                            String fullName = instructor.get("full_name");
                            return idNumber != null && fullName != null ? idNumber + " - " + fullName : "";
                        })
        ...
        courseComboBox.setItems(FXCollections.observableArrayList(
                courses.stream()
                        .map(course -> {
                            String courseCode = course.get("course_Code");
                            String courseName = course.get("course_name");
                            return courseCode != null && courseName != null ? courseCode + " - " + courseName : "";
                        })
                        .filter(item -> !item.isEmpty())
                        .toList()));
