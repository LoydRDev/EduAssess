...
public class CourseDAO {
    ...
    public User getStudentById(String studentId) {
        String sql = "SELECT s.student_IDNumber, s.student_Name, u.email, s.student_Type FROM student s JOIN users u ON s.student_IDNumber = u.id_number WHERE s.student_IDNumber = ?";
        try (Connection conn = DatabaseConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, studentId);
            try (ResultSet rs = stmt.executeQuery()) {
    ...
    public boolean updateStudentType(String studentId, String studentType) {
        String sql = "UPDATE student SET student_Type = ? WHERE student_IDNumber = ?";
